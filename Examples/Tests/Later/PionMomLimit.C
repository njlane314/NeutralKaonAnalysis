...
void PionMomLimit(){
    ...
    for(int i = 0; i < E.GetNEvents(); i++){
        ...
        for(size_t i = 0; i < e.TracklikePrimaryDaughters.size(); i++){
            ...
            if(abs(trackPDG) == 211){ // 13 muon, 211 pion, 2212 proton
                ...
                if(trackTrueMomentum > 0.08 && trackTrueMomentum < 0.12) {
                    hMuonMomentumBias1->Fill(bias);
                }
                else if(trackTrueMomentum > 0.12 && trackTrueMomentum < 0.16) {
                    hMuonMomentumBias2->Fill(bias);
                }
                else if(trackTrueMomentum > 0.16 && trackTrueMomentum < 0.2) {
                    hMuonMomentumBias3->Fill(bias);
                }
                else if(trackTrueMomentum > 0.2 && trackTrueMomentum < 0.25) {
    ...
    hMuonMomentumBias1->SetMaximum(0.09);
    ...
    legend->AddEntry(hMuonMomentumBias1, "p_{#pi, true} #in [0.08, 0.12] GeV/c", "l");
    legend->AddEntry(hMuonMomentumBias2, "p_{#pi, true} #in [0.12, 0.16] GeV/c", "l");
    legend->AddEntry(hMuonMomentumBias3, "p_{#pi, true} #in [0.16, 0.20] GeV/c", "l");
    //legend->AddEntry(hMuonMomentumBias4, "p_{#pi} #in [0.25, 0.30] GeV/c", "l");
    legend->SetTextSize(0.03);
    legend->SetBorderSize(0); 
    legend->SetFillStyle(0);  
    legend->Draw();

    c1->SaveAs("pion_momentum_limit.png");
