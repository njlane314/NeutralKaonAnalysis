...
void ShowerlikePrimaryDaughters(){

	double POT = 1.0e21;
	std::string label = "showers";
	...
	M.SetupHistograms(10, 0, 10, ";Showers;Events");
	...
	for(size_t s_itr = 0; s_itr < SampleNames.size(); s_itr++){
		...
		for(int evt_itr = 0; evt_itr < E.GetNEvents(); evt_itr++){
			...
			M.FillHistograms(evt, evt.ShowerlikePrimaryDaughters.size());
		}   
